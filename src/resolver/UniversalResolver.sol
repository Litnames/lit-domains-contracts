 ...
 * making it possible to make a single smart contract call to resolve an LNS name.
 */
contract UniversalResolver is ERC165, Ownable {
    ...
    /**
     * @dev Performs LNS name resolution for the supplied name and resolution data.
    ...
    function _resolve(
        ...
        (LitDefaultResolver resolver,, uint256 finalOffset) = findResolver(name);
    ...
    /**
     * @dev Performs LNS name reverse resolution for the supplied reverse name. Called by Viem when doing getEnsName()
    ...
    /**
     * @dev Performs LNS name reverse resolution for the supplied reverse name.
     * @param reverseName The reverse name to resolve, in normalised and DNS-encoded form. e.g. b6E040C9ECAaE172a89bD561c5F73e1C48d28cd9.addr.reverse
     * @return The resolved name, the resolved address, the reverse resolver address, and the resolver address.
     */
     ...
     */
    function findResolver(bytes calldata name) public view returns (LitDefaultResolver, bytes32, uint256) {
        (address resolver, bytes32 namehash, uint256 finalOffset) = findResolver(name, 0);
        return (LitDefaultResolver(resolver), namehash, finalOffset);
    }
    ...
    function _hasExtendedResolver(address resolver) internal view returns (bool) {
        try LitDefaultResolver(resolver).supportsInterface{gas: 50_000}(type(IExtendedResolver).interfaceId) returns (
            bool supported
        ) {
            return supported;
        } catch {
